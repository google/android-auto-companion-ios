...
public class TrustAgentManager: FeatureManager {
  ...
  init(
    ...
    // Whenever the device is unlocked, resend the credentials for the connected cars.
      ...
      deviceUnlockObserver = NotificationCenter.default.addObserver(
        ...
        Self.logger.log("Device Unlocked - sending credentials for connected cars.")
  ...
  private func enroll(_ car: Car, isInitiatedFromCar: Bool) throws {
    guard config.arePasscodeRequirementsMet() else {
      Self.logger.error.log("Cannot enroll since passcode has not been set on this device.")
    ...
    guard let escrowToken = escrowTokenManager.generateAndStoreToken(for: car.id) else {
      Self.logger.error.log("Error generating and storing escrow token.")
  ...
  public override func onSecureChannelEstablished(for car: Car) {
    ...
    Self.logger.log("Car \(car.logName) connected securely. Currently enrolled: \(enrolled)")
  ...
  public override func onCarDisassociated(_ car: Car) {
    Self.logger.log("Car \(car.logName) disassociated.")
  ...
  public override func onMessageReceived(_ message: Data, from car: Car) {
    guard let trustedDeviceMessage = try? TrustedDeviceMessage(serializedData: message) else {
      Self.logger.error.log("Failed to decode message from serialized data.")
  ...
  private func clearEnrollment(for car: Car, syncToCar: Bool, initiatedFromCar: Bool) {
    Self.logger.log(
      "Clearing enrollment status for (\(car.logName)). Removing any tokens and handles.")
  ...
  private func syncDisabledFeatureStatus(with car: Car) {
    guard let featureStatus = try? makeDisabledFeatureStatus() else {
      Self.logger.error.log(
        "Unable to create disabled feature status to send to car \(car.logName).")
      return
    }

    if securedCars.contains(car) {
      Self.logger.log("Car \(car.logName) currently connected. Syncing feature status with it.")
  ...
  private func sendFeatureStatus(_ featureStatus: Data, to car: Car) -> Bool {
    do {
      ...
      Self.logger.error.log("Unable to send feature status to car: \(error.localizedDescription)")
      return false
    }
  }
  ...
  private func sendEscrowToken(_ escrowToken: Data, to car: Car) {
    do {
      ...
      Self.logger.error.log("Error when sending escrow token to car: \(car.logName)")
  ...
  private func maybeSyncFeatureStatus(with car: Car) {
    guard let featureStatus = trustAgentStorage.featureStatus(for: car) else {
      Self.logger.debug.log(
        "Car \(car.logName) does not have any stored feature status messages to send.")
      return
    }

    Self.logger.log(
      "Car \(car.logName) has stored feature status messages to sync. Sending to car.")
  ...
  private func sendPhoneCredentials(to car: Car) {
    guard let token = escrowTokenManager.token(for: car.id) else {
      Self.logger.log("No token found for car \(car.logName). Meaning it is not enrolled")
      return
    }

    guard let handle = escrowTokenManager.handle(for: car.id) else {
      Self.logger.log("No handle found for car \(car.logName). Meaning it is not enrolled")
      return
    }
    ...
    guard config.areLockStateRequirementsMet(for: car) else {
      Self.logger.error.log("Cannot send unlock credentials since this device is locked.")
    ...
    guard let phoneCredentials = try? makePhoneCredentials(token: token, handle: handle) else {
      Self.logger.error.log("Unable to combine token and handle to be sent to car \(car.logName).")
    ...
    guard isCarSecurelyConnected(car) else {
      Self.logger.log(
        "Car (\(car.logName)) to use for sending unlock credentials is invalid. Ignoring.")
      return
    }
    ...
    do {
      ...
      Self.logger.error.log("Error sending unlock credentials to car (\(car.logName))")
  ...
  private func handleTrustedDeviceMessage(_ message: TrustedDeviceMessage, from car: Car) {
    Self.logger.debug.log("Received message of type \(message.type.rawValue).")
    ...
    default:
      Self.logger.error.log("Unhandled message type \(message.type.rawValue)")
    }
  }
  ...
  private func handleAcknowledgmentMessage(from car: Car) {
    ...
    if config.isUnlockHistoryEnabled {
      ...
      Self.logger.debug.log("Unlock history enabled. Storing unlock date: \(unlockDate)")
    }
  ...
  private func processHandle(_ handle: Data, from car: Car) {
    Self.logger.debug.log("Received handle from head unit.")

    guard escrowTokenManager.storeHandle(handle, for: car.id) else {
      Self.logger.error.log("Error during storage of handle.")
      delegate?.trustAgentManager(
  ...
  private func sendHandleReceivedConfirmation(to car: Car) {
    Self.logger.log("Writing confirmation that handle has been received.")
    ...
    do {
      ...
      Self.logger.error.log(
        "Cannot send confirmation message for association completion for sending")
  ...
  private func syncFeatureStatus(_ featureStatus: Data, from car: Car) {
    guard let status = try? TrustedDeviceState(serializedData: featureStatus) else {
      Self.logger.error.log("Unable to parse feature status from \(car.logName). Ignoring.")
      return
    }

    Self.logger.debug.log("Received feature status from \(car.logName). Enabled: \(status.enabled)")
