...
public class TrustAgentManager: FeatureManager {
  ...
  private var deviceUnlockObserver: Task<Void, Never>?
  ...
  init(
    ...
    #if os(iOS)
      // Handling user initiated unlock events, so give it top priority.
      deviceUnlockObserver = Task(priority: .userInitiated) { @MainActor [weak self] in
