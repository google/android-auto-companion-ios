...
/**
 * Returns the Messaging Style User.
 * This field is required field for all messaging styles
 * and is an easy way to indicate if a message style exists
 *
 * @return notification extras value for Messaging Style User
 * @see NotificationCompat.MessagingStyle.extractMessagingStyleFromNotification
 */
val Notification.messageUserDisplayName: CharSequence?
  get() = extras.getCharSequence(NotificationCompat.EXTRA_SELF_DISPLAY_NAME)

/**
 * Returns true if is car-compatible messaging notification
 * Removing [markAsReadAction] non-null check requirement to support Hangouts
 * which, from the last manual test, does not have Mark As read action (b/145631112)
 */
val Notification.isCarCompatibleMessagingNotification: Boolean
  get() = messageUserDisplayName != null &&
    replyAction != null && markAsReadAction != null && !showsUI
