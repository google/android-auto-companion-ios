...
private val DATA_PARCEL_UUID = ParcelUuid(AssociationManager.DEVICE_NAME_DATA_UUID)
...
class AssociationManagerTest {
  ...
  fun onScanResult_returnsCorrectDiscoveredCar() {
    ...
    val deviceName = ByteArray(2)
    val serviceData = hashMapOf(DATA_PARCEL_UUID to deviceName)

    val scanRecord =
      createScanRecord(
        name = "backupName",
        serviceUuids = listOf(ParcelUuid(TEST_UUID)),
    ...
    val expectedCar =
      DiscoveredCar(scanResult.device, "$TEST_PREFIX${deviceName.toHexString()}", TEST_UUID, null)
  ...
  fun onScanResult_returnScanRecordName_ifServiceDataDoesNotHaveName() {
    ...
    val scanRecord =
      createScanRecord(
        name = deviceName,
        serviceUuids = listOf(ParcelUuid(TEST_UUID)),
        serviceData = hashMapOf()
      )

    val scanResult = createScanResult(scanRecord)
    bleManager.triggerOnScanResult(scanResult)

    val expectedCar = DiscoveredCar(scanResult.device, deviceName, TEST_UUID, null)
    verify(discoveryCallback).onDiscovered(expectedCar)
  }
  ...
  fun onScanResult_doesNotInvokeCallback_ifWrongServiceUuid() {
    ...
    val serviceData = hashMapOf(DATA_PARCEL_UUID to "deviceName".toByteArray())
    val scanRecord =
      createScanRecord(
        name = null,
        serviceUuids = listOf(ParcelUuid(invalidUuid)),
        serviceData = serviceData
      )
  ...
  fun onScanResult_doesNotInvokeCallback_ifMultipleServiceUuids() {
    ...
    val scanRecord =
      createScanRecord(
        name = null,
        serviceUuids = listOf(ParcelUuid(TEST_UUID), ParcelUuid(invalidUuid)),
        serviceData = serviceData
      )
