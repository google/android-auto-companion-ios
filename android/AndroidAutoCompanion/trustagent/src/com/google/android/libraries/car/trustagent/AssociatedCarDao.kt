...
internal interface AssociatedCarDao {
  ...
  /** Deletes an associated car that has the given [carId] */
  @Query("DELETE FROM associated_cars WHERE id = :carId")
  suspend fun deleteAssociatedCar(carId: String)

  /** Deletes all currently associated cars. */
  @Query("DELETE FROM associated_cars") suspend fun deleteAllAssociatedCars()
}
