...
internal constructor(
  ...
  open suspend fun filterForConnectableCars(scanResults: List<ScanResult>): List<ScanResult> {
    ...
    logi(TAG, "Always connecting to device with names: $allowedDeviceNames")
  ...
  companion object {
    ...
    internal fun checkPermissionsForBleScanner(context: Context): Boolean {
      ...
      if (!checkPermission(context, ACCESS_FINE_LOCATION) &&
        ...
        loge(TAG, "Missing fine/coarse location permission. No-op")
        return false
      }
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        if (!checkPermission(context, ACCESS_BACKGROUND_LOCATION)) {
    ...
    private fun checkPermission(context: Context, permission: String): Boolean =
      (context.checkSelfPermission(permission) == PERMISSION_GRANTED).also { granted ->
