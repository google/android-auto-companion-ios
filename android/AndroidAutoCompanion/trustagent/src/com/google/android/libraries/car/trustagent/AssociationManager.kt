...
internal constructor(
  ...
  /**
   * The context for executing any [ListenableFuture]s.
   *
   * This context should be capable of running in the background since it will be responsible for
   * accessing the database.
   */
  @VisibleForTesting
  internal var backgroundContext: CoroutineDispatcher =
  ...
  fun startDiscovery(filterService: UUID, namePrefix: String = ""): Boolean {
    ...
    if (!ConnectionManager.checkPermissionsForBleScanner(context)) {
  ...
  /**
   * Asynchronously clears the association status for the car with the given [deviceId] and
   * [CompanionDeviceManager] association with the device.
   */
  internal fun clearCdmAssociatedCar(deviceId: UUID) {
    logi(TAG, "Request to clear association for Cdm associated car with device id $deviceId")
