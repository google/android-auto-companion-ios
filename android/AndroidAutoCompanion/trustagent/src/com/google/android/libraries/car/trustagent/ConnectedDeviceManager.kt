...
class ConnectedDeviceManager(
  ...
  private val associationManagerCallback =
    object : AssociationManager.AssociationCallback, AssociationManager.DisassociationCallback {
      ...
      override fun onAllCarsDisassociated() {
        ...
        callbacks.forEach { it.onAllCarsDisassociated() }
      }
    }
  ...
  fun disassociate(deviceId: UUID) {
    associationManager.clearCdmAssociatedCar(deviceId)
  }

  /** Asynchronously clears all cars that are currently associated with this device. */
  fun disassociateAllCars() {
  ...
  interface Callback {
     ...
     *
     * This method is called when when the [disassociate] is called, as opposed to
     * [disassociateAllCars], which clears all associated cars. The latter will instead trigger the
     * [onAllCarsDisassociated] callback.
     */
    fun onDisassociated(deviceId: UUID)
     ...
     *
     * This method is only invoked when the [disassociateAllCars] method is called, as opposed to
     * [disassociate]. The latter will instead trigger the [onDisassociated] callback.
     */
